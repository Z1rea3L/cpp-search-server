...
public:
    ...
    void AddDocument(int document_id, const string& document) {
        ...
        for(const string& str: words){
            word_to_document_freqs_[str][document_id] += 1.0/words.size();
        }
        ++document_count_;
    }
...
private:
    ...
    vector<Document> FindAllDocuments(const Query& query) const {
        ...
        if(query.minus_query.size()!=0){
            for(const string& word : query.minus_query){
                if(word_to_document_freqs_.count(word)==0){
                    continue;
                }
                for(const auto& [id, tf] : word_to_document_freqs_.at(word)){
        ...
        for (const auto& [id, tf_idf] : document_to_relevance) {
            if(tf_idf>=0)matched_documents.push_back({id, tf_idf});
        }
        return matched_documents;
    }
};
